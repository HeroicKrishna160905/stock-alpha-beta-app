...
if st.button("Run Analysis"):
    ...
    if data.empty:
        ...
        c1, c2 = st.columns(2)
        c1.metric("Beta Mean", f"{rb.mean():.4f}")
        c1.metric("Beta Std", f"{rb.std():.4f}")
        c2.metric("Alpha Mean", f"{ra.mean():.4f}")
        c2.metric("Alpha Std", f"{ra.std():.4f}")
        ...
        def runs_test(x):
            ...
            std = np.sqrt((2 * n1 * n2 * (2 * n1 * n2 - n1 - n2)) /
                          (((n1 + n2) ** 2) * (n1 + n2 - 1))) if n1 + n2 - 1 > 0 else 0
            z = (runs - exp) / std if std > 0 else 0
        ...
        st.write(f"- **T-test** (Beta ≠ 0) p = {t_b.pvalue:.4f}, (Alpha ≠ 0) p = {t_a.pvalue:.4f}")
        st.write(f"- **Runs Test** (Beta) z = {rt_b[2]:.2f}, p = {rt_b[3]:.4f}")
        st.write(f"- **Runs Test** (Alpha) z = {rt_a[2]:.2f}, p = {rt_a[3]:.4f}")
        ...
        if simulate:
            ...
            sim_steps = st.slider(
                "Simulation Length (Number of Windows)",
                min_value=1,
                max_value=60,
                value=sim_steps if 'sim_steps' in locals() else  years,  # fallback
                help="How many rolling-window steps to simulate"
            )
            ...
            for i in range(num_simulations):
                sims_alphas = np.random.normal(ap_mean, ap_std, sim_steps)
                p = [initial]
                for a in sims_alphas:
            ...
            ax2.set_title(
                f"Simulated Compounded Amount over {sim_steps} Steps "
